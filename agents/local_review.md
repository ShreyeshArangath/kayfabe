...
3. **Set up the remote and worktree**:
   ...
   - If not, add it: `git remote add USERNAME git@github.com:USERNAME/workspace-app`
   - Fetch from the remote: `git fetch USERNAME`
   - Create worktree: `git worktree add -b BRANCHNAME ~/wt/app/SHORT_NAME USERNAME/BRANCHNAME`

4. **Configure the worktree**:
   - Copy any required workspace settings files (e.g., `settings.local.json`) into the worktree so tooling matches the main repo
   - Run setup: `make -C WORKTREE setup`
   - Initialize thoughts: `cd WORKTREE && thoughts init --directory app`
...
- Create worktree at `~/wt/app/eng-1696`
