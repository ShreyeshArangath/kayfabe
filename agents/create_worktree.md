...
2a. read `hack/create_worktree.sh` and create a new worktree using the Jira issue key in the path: `./hack/create_worktree.sh JIRA-XXXX BRANCH_NAME`
...
worktree path: ~/wt/app/ENG-XXXX
branch name: BRANCH_NAME
path to plan file: $FILEPATH
launch prompt:

    /implement_plan at $FILEPATH and when you are done implementing and all tests pass, read ./claude/commands/commit.md and create a commit, then read ./claude/commands/describe_pr.md and create a PR, then add a comment to the Jira issue with the PR link
```
